...
void HLSLExternalSemaSource::defineHLSLTypesWithForwardDeclarations() {
  ...
  onCompletion(Decl, [this](CXXRecordDecl *Decl) {
    auto *Template = Decl->getDescribedClassTemplate();
    assert(Template && "ConstantBuffer must be a template");
    TemplateTypeParmDecl *T = cast<TemplateTypeParmDecl>(
        Template->getTemplateParameters()->getParam(0));
    QualType TType = Decl->getASTContext().getTypeDeclType(T);
