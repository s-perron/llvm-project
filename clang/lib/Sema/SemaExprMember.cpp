      ...
      if (RD->getName() == "ConstantBuffer") {
        if (auto *CTSD = dyn_cast<ClassTemplateSpecializationDecl>(RD)) {
          if (CTSD->getTemplateArgs().size() > 0) {
            ...
            QualType ReturnTy = S.Context.getCanonicalType(S.Context.getLValueReferenceType(AddrSpaceType));
            ...
            if (S.LookupQualifiedName(ConvR, RD)) {
              ...
              for (NamedDecl *D : ConvR) {
                if (auto *CD = dyn_cast<CXXConversionDecl>(D->getUnderlyingDecl())) {
                  ConvDecl = CD;
                  FoundDecl = D;
                  break;
                }
              ...
              if (ConvDecl) {
                ExprResult ConvCall = S.BuildCXXMemberCallExpr(BaseExpr.get(), FoundDecl, ConvDecl, /*HadMultipleCandidates=*/false);
                if (!ConvCall.isInvalid()) {
                  BaseExpr = ConvCall;
                  return LookupMemberExpr(S, R, BaseExpr, IsArrow, OpLoc, SS, ObjCImpDecl, HasTemplateArgs, TemplateKWLoc);
                }
              }
            }
          }
