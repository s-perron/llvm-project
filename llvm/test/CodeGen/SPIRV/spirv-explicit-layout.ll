...
; CHECK-DAG: [[buffer_var:%[0-9]+]] = OpVariable [[buffer_ptr]] StorageBuffer
...
; This case adds fails because `deduceElementTypeByValueDeep` seems to
; return the wrong type for the pointer operand, and causes an invalid
; pointer case to be inserted.
;
; https://github.com/llvm/llvm-project/blob/1b86867ab3ccf731863043718d53700535e0fb21/llvm/lib/Target/SPIRV/SPIRVEmitIntrinsics.cpp#L1555
;
; ; CHECK-DISABLE: OpFunction [[S]] None
; define external void @vulkan_buffer_store() {
; ; CHECK-DISABLE-NEXT: OpLabel
; entry:
; ; CHECK-DISABLE-NEXT: [[handle:%[0-9]+]] = OpCopyObject [[buffer_ptr]] [[buffer_var]]
;   %handle = tail call target("spirv.VulkanBuffer", [0 x %struct.S], 12, 0) @llvm.spv.resource.handlefrombinding(i32 0, i32 0, i32 1, i32 0, i1 false)
;
; ; CHECK-DISABLE-NEXT: [[ptr:%[0-9]+]] = OpAccessChain [[storagebuffer_S_ptr]] [[handle]] [[zero]] [[one]]
;   %0 = tail call noundef nonnull align 4 dereferenceable(4) ptr addrspace(11) @llvm.spv.resource.getpointer(target("spirv.VulkanBuffer", [0 x %struct.S], 12, 0) %handle, i32 1)
;
; ; CHECK-DISABLE-NEXT: [[copy:%[0-9]+]] = OpCopyLogical [[S_explict]] [[null_S]]
; ; CHECK-DISABLE-NEXT: OpStore [[ptr]] [[copy]] Aligned 4
;   store %struct.S zeroinitializer, ptr addrspace(11) %0, align 4
;
; ; CHECK-DISABLE-NEXT: OpReturn
;   ret void
;
; ; CHECK-DISABLE-NEXT: OpFunctionEnd
; }