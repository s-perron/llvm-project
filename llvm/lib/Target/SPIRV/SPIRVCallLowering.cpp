...
bool SPIRVCallLowering::lowerIntrinsicCall(MachineIRBuilder &MIRBuilder,
  ...
  if (const IntrinsicInst *II = dyn_cast<IntrinsicInst>(&Call)) {
    if (II->getIntrinsicID() == Intrinsic::spv_assign_type) {
      Value *Val = II->getOperand(0);
      MetadataAsValue *MD = cast<MetadataAsValue>(II->getOperand(1));
      Type *Ty = cast<ConstantAsMetadata>(MD->getMetadata())->getType();

      Register ValReg = FLI.getRegForValue(Val);
      if (ValReg.isValid()) {
        SPIRVType *SpirvTy = GR->getOrCreateSPIRVType(
            Ty, MIRBuilder, SPIRV::AccessQualifier::ReadWrite, true);
        GR->assignSPIRVTypeToVReg(SpirvTy, ValReg, MIRBuilder.getMF());
        return true;
      }
    } else if (II->getIntrinsicID() == Intrinsic::spv_assign_ptr_type) {
      Value *Val = II->getOperand(0);
      MetadataAsValue *MD = cast<MetadataAsValue>(II->getOperand(1));
      Type *Ty = toTypedPointer(
          cast<ConstantAsMetadata>(MD->getMetadata())->getType());
      int AddressSpace = cast<ConstantInt>(II->getOperand(2))->getZExtValue();
      const SPIRVSubtarget &STI =
          MIRBuilder.getMF().getSubtarget<SPIRVSubtarget>();
      SPIRV::StorageClass::StorageClass SC =
          addressSpaceToStorageClass(AddressSpace, STI);

      Register ValReg = FLI.getRegForValue(Val);
      if (ValReg.isValid()) {
        SPIRVType *SpirvTy =
            GR->getOrCreateSPIRVPointerType(Ty, MIRBuilder, SC);
        GR->assignSPIRVTypeToVReg(SpirvTy, ValReg, MIRBuilder.getMF());
        return true;
      }
    }
