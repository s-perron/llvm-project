...
SPIRVType *SPIRVGlobalRegistry::getOrCreateUnknownType(
  ...
  if (Opcode == 32) {
    uint32_t SC =
        cast<llvm::TargetExtType>(Ty->getTypeParameter(0))->getIntParameter(0);
    SPIRVType *PointeeType =
        getOrCreateSPIRVType(Ty->getTypeParameter(1), MIRBuilder);
    return getOrCreateSPIRVPointerType(
        PointeeType, MIRBuilder,
        static_cast<SPIRV::StorageClass::StorageClass>(SC));
  }
