...
Register SPIRVInstructionSelector::buildPointerToResource(
  ...
  if (ArraySize == 1) {
    const Type *T = GR.getTypeForSPIRVType(ResType);
    SPIRVType *PtrType =
        GR.getOrCreateSPIRVPointerType(const_cast<Type *>(T), MIRBuilder, SC);
    assert(GR.getPointeeType(PtrType) == ResType &&
           "ResType did not have an explicit layout.");
